    ...
    <section class="c-section-info">
        <div class="c-hero">
            <div class="c-hero_image">
                <img class="o-media-wrapper" src="{{asset('img/sklad.png')}}" width="1920" height="1080">
            </div>
        </div>
        <div class="name_page-style">Корзина</div>
    </section>
    @if(count($CartList) > 0)
        <div class="cart-page-main container-md">
                    ...
                    @foreach($position as $pos)
                        @if($pos -> package_name === $cart -> attributes -> package_name and $pos -> model_name === $cart -> name)
                            <div class="cart-block-row2">
                                <div class="del_assemblies del_assemblies-{{ $pos -> id }}" data-id="{{ $pos -> id }}" title="Удалить товар из корзины">✖</div>
                                <div class="cart-block-row2_product-block">
