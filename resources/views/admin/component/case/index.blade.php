    ...
    <main class="o-main">
        ...
        <div class="admin-main-content container">
            ...
            <div class="pt-5 pb-3 fs-3 heading-functional">
                {{--Действие--}}
                Управление комплектующими: <span class="fs-3 fw-bold">{{ $models -> name }} - Корпусы</span>
            </div>
            <div class="d-flex flex-wrap admin-action-content">
                {{--Корпуса--}}
                <container>
                    <div class="fs-3 pb-2">Корпусы</div>
                    {{--Добавление корпуса--}}
                    <container>
                        <!-- Button trigger modal -->
                        <button type="button" class="btn btn-primary mb-2" data-bs-toggle="modal" data-bs-target="#CaseModal">
                            Добавить корпус
                        </button>

                        <!-- Modal -->
                        <div class="modal fade" id="CaseModal" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="staticBackdropLabel">Добавление корпуса</h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                    </div>
                                    <form action="{{ route('admin.components.case.store') }}" method="post" enctype="multipart/form-data">
                                        @csrf
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <div class="mb-2">
                                                ...
                                                <select class="form-select" name="size">
                                                    <option selected disabled>Выберите размер корпуса</option>
                    ...
                    <div class="d-flex flex-wrap mb-5" style="column-gap: 130px; row-gap: 30px;">
                        @foreach($cases as $case)
                            @if($case -> ComputerModel -> name === $models -> name)
                                <div class="card" style="width: 22rem;">
                                    <div class="model-img">
                                        <img src="{{ asset('storage/img/component_photo/case/' . $case -> img ) }}" class="pt-3 w-100 h-100" alt="{{ $case -> name }}">
                                    </div>
                                    <div class="card-body">
                                        ...
                                        <container>
                                            <!-- Button trigger modal -->
                                            <button type="button" class="btn btn-primary mb-2" data-bs-toggle="modal" data-bs-target="#CaseModal{{ $case -> id }}">
                                                Редактировать
                                            </button>

                                            <!-- Modal -->
                                            <div class="modal fade" id="CaseModal{{ $case -> id }}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                                                <div class="modal-dialog">
                                                    <div class="modal-content">
                                                        <div class="modal-header">
                                                            <h5 class="modal-title" id="staticBackdropLabel">Изменение корпуса</h5>
                                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                                        </div>
                                                        <form action="{{ route('admin.components.case.update', $case -> id )}}" method="post" enctype="multipart/form-data">
                                                            @csrf
                                                            @method('PATCH')
                                                            <div class="modal-body">
                                                                {{--Название--}}
                                                                ...
                                                                <div class="mb-2">
                                                                    <label class="form-label">Модель</label>
                                                                    <input class="form-control" type="text" name="model" value="{{ $case -> model }}">
                                                                </div>
                                                                {{----}}
                                                                ...
                                                                <div class="mb-2">
                                                                    <label class="form-label">Тип</label>
                                                                    <select class="form-select" name="size">
                                                                        <option selected disabled>{{ $case -> size }}</option>
                                                                        <option value="Full-Tower">Full-Tower</option>
                                                                        <option value="Mid-Tower">Mid-Tower</option>
                                                                        <option value="Mini-Tower">Mini-Tower</option>
                                                                    </select>
                                                                ...
                                                                <div class="mb-2">
                                                                    <label class="form-label">Цена</label>
                                                                    <input class="form-control" type="number" name="price" value="{{ $case -> price }}">
                                                                </div>
                                                                {{----}}
